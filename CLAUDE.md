...
# Testing
npm test               # Run all tests (885 tests, 60 suites — must be 0 failures)
...
- Employee-catalog conflict detection: declarative pure functions in `src/utils/employeeConflicts.js` and `src/utils/conflictHelpers.js`. `detectEmployeeConflicts(modes)` returns array of `{ workerId, modeIdA, modeIdB, day, startMin, endMin }` computed runtime — conflicts are never persisted. Helpers expose description formatting, avatar computation for calendar cells, and per-cell conflict check. Modal: `src/components/EmployeeConflictModal.js` renders resolution UI with three options for newAssignment variant (PR #92). WeekCalendarView renders worker avatars (max 3 + overflow count) in each band with red border + alert icon on conflict cells (PR #93). Tab Horario integration with rollback options comes in PR #94. Pattern documented in `docs/architecture_design/employee_catalog_conflicts.md`.
...
- **Rule: 885 tests (minimum), 0 failures before any merge. No exceptions.**
